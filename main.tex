...
%However, free energy decompositions can be problematic. Obtaining a decomposition may be easy, but obtaining a meaningful or interpretable decomposition can be hard. There are two primary challenges. First, free energy decompositions are path dependent. Although the total free energy ∆A is a state function, it’s contributions are not, meaning that different paths lead to different decompositions. Second, free energy changes are often non-additive. The available phase space of the system depends strongly on the Hamiltonian. Changes to one component of the Hamiltonian can completely change the available phase space, thus strongly influencing the values of other components.


%Free energy calculations are an increasingly important tool of molecular simulation. An attractive prospect is to break a single free energy value into separate additive components. For example, in protein-protein binding, we may be interested in breaking the binding free energy into per-residue free energy contributions that could be used to identify `hot spots' to guide mutations and design.  [Other examples, references that use free energy decompositions. Free energy decomps are used in chem engineering~\cite{NULL}]

%Unfortunately, these free energy components are themselves not state functions because, aside from idealized cases, a free energy cannot be separated into additive components. For this reason, it is unclear exactly how these free energy components should be interpreted. The central problem is that a free energy value contains \textit{couplings} between the different degrees of freedom that would be partitioned across the different free energy components. In general, it is unclear how these couplings are `assigned' to the different free energy components. For this reason, the free energy components are in general \textit{path dependent}, where the allocation of these coupled interactions into any given free energy component will depend on the alchemical path. These combined issues make it unclear exactly how these free energy components should be \textit{interpreted}; i.e., what in their physical meaning, and how can they be used to make predictions?  Mark and Van Gunsteren have used these arguments to conclude that free energy decompositions are inherently meaningless and should not be used. Karplus and coworkers have argued that in many cases the couplings may be \textit{small} and thus free energy components still provide useful information if interpreted carefully~\cite{NULL}. 
...
This is an intriguing result, and states that any change in component $H_0$ is exactly cancelled by the coupling of $H_0$ to the scaled components $H_1$ and $H_2$, see appendix~\warning{NULL}. 
 ...
 %That these terms cancel is not immediately obvious, see appendix \warning{NULL}. 
%This expansion readily generalizes to an arbitrary number of Hamiltonian components. 
