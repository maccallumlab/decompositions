...
We can use these expressions to predict the free energy change in the absence of certain components of the Hamiltonian. For example, we predict that the free energy change when $H_2=0$ will be:
\begin{align*}
A_{H_2=0}' =& A_1' + A_{1,2}'\\
=&
\frac
...
A couple of points:
\begin{itemize}
	\item I'm confused about why it isn't just $A_1$ instead of $A_1 + A_{1,2}$, but this is what gives the correct answer.
	\item This is exact. There is no non-additivity here.
	\item The exponential may make this noisy and hard to estimate.
	\item There will be a similar result for the truncated cumulant expansion that may give more accurate results with finite sampling.
\end{itemize}

Evidently, this also implies that the sum of the cumulants that involve $H_1$ under the Hamiltonian $H = H_1 + H_2$ is equal to the sum of the cumulants involving $H_1$ under the Hamiltonian $H = H_1$, see Figure~\ref{fig:sum}. How does this generalize to more components? This seems really interesting to me. It reminds me of the case in the old writeup, where all of the change to the spectator component would cancel. In that case, we were also using a splitting where every term that contains $H_0$ went to the same component.

\begin{figure}[tb]
\centering
\includegraphics[width=3in]{figure7.pdf}
\caption{Sums of cumulants are the same under different ensembles.}
\label{fig:sum}
\end{figure}

These results also suggest another decomposition, where:
\begin{align*}
{A_1^{*}}' =&
	A_1' + A_{1,2}' \\
=&
...
=&
	...
	\frac
		...
		{\left\langle e^{\beta H_2} \right\rangle_\lambda}
\end{align*}
This might be more interpretable than the one before. It's a bit weird though, because the splitting coefficients are no longer in the range $[0, 1]$, instead some of them are now negative.
